...
test("every persisted card color has one CSS token", () => {
  const css = readFileSync(new URL("../styles/card-colors.css", import.meta.url), "utf8");
  ...
  for (let number = 1; number <= CARD_COLOR_COUNT; number++) assert.equal(cardColorNumber(number), number);
  assert.equal(cardColorNumber("#2765ec"), cardColorNumber("#2765EC"));
  for (const invalid of [0, 43, -1, 1.5, "1", "red", null, undefined]) assert.equal(cardColorNumber(invalid), null);
});
