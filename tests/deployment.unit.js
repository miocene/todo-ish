...
test("deployment stages committed manifest inputs without ignored notes or working-tree edits", (t) => {
  ...
  assert.equal(
    readFileSync(join(destination, "src/app.js"), "utf8"),
    "committed",
  );
  ...
  assert.equal(
    readFileSync(join(destination, "REVISION"), "utf8"),
    revision + "\n",
  );
});
...
test("deploy reads local Pi settings, honors environment overrides and forwards checksum adoption", (t) => {
  ...
  t.after(() => rmSync(root, { recursive: true, force: true }));
  for (const directory of ["backend/scripts", "tools", "bin"])
    mkdirSync(join(root, directory), { recursive: true });
  const script = join(root, "backend/scripts/deploy_raspberry_pi.sh");
  writeFileSync(
    script,
    readFileSync(
      new URL("../backend/scripts/deploy_raspberry_pi.sh", import.meta.url),
    ),
  );
  const revision = "a".repeat(40);
  writeFileSync(
    join(root, "tools/stage-release.mjs"),
  ...
  const env = {
    ...
    DEPLOY_CALLS_FILE: log,
  };
  for (const key of [
    "PI_SSH_TARGET",
    "PI_LAN_ADDRESS",
    "PI_APP_DIR",
    "PI_ADOPT_LEGACY_CHECKSUMS",
  ])
    delete env[key];
  const run = (overrides = {}) =>
  ...
  writeFileSync(log, "");
  assert.match(
    run({ PI_ADOPT_LEGACY_CHECKSUMS: "0", PI_SSH_TARGET: "override@pi.local" }),
    /checksum adoption: 0/,
  );
  assert.match(
    readFileSync(log, "utf8"),
    /override@pi.local MIGRATION_ADOPT_LEGACY_CHECKSUMS=0/,
  );
  writeFileSync(log, "");
  assert.throws(
    () => run({ PI_ADOPT_LEGACY_CHECKSUMS: "yes" }),
    /must be 0 or 1/,
  );
