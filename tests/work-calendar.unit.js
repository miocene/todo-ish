...
import { calendarDate, getWorkDateBounds, isoDate, requestedDate } from "../src/app/work-calendar.js";
...
test("old completed history is viewable without expanding the assignment window", () => {
  ...
  assert.equal(isoDate(bounds.firstAssignableDate), "2026-09-11");
  assert.equal(isoDate(requestedDate("2026-07-01", calendarDate("2026-09-14"), bounds)), "2026-08-01");
  assert.equal(isoDate(requestedDate("2026-12-01", calendarDate("2026-09-14"), bounds)), "2026-09-28");
  assert.equal(isoDate(requestedDate("2026-02-30", calendarDate("2026-09-14"), bounds)), "2026-09-14");
});
...
test("backdating uses calendar days across year and daylight-saving boundaries", () => {
  for (const [today, expected] of [
    ...
    assert.equal(isoDate(getWorkDateBounds(calendarDate(today)).firstAssignableDate), expected);
  }
});
