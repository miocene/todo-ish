...
for (const status of [0, 400, 503]) {
  test(`registration failure ${status} preserves the created passkey`, async ({
    page,
    appData,
  }) => {
    appData.setSession({
    ...
    await page.route("**/api/auth/registration/verify", (route) =>
      status
        ? route.fulfill({
            status,
            json: { error: "Rejected", code: "registration_failed" },
          })
        : route.abort(),
    );
    ...
    await expect(page.getByRole("alert")).toContainText(
      status === 400 ? "Rejected" : "Try signing in",
    );
...
test("unknown sign-in credentials use the ceremony RP ID", async ({
  page,
  appData,
}) => {
  appData.setSession({
  ...
  await page.route("**/api/auth/authentication/options", (route) =>
    route.fulfill({ json: { rpId: "example.test" } }),
  );
  await page.route("**/api/auth/authentication/verify", (route) =>
    route.fulfill({
      status: 404,
      json: { error: "Missing", code: "credential_not_found" },
    }),
  );
  await page.goto("/work");
  await page
