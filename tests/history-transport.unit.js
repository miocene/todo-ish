...
test("large histories produce bounded edits and preserve multilingual titles", () => {
  ...
  const next = {
    ...previous,
    history: [...history, { id: "new", title: "Готово", completedAt }].slice(1),
  };
  const delta = validateHistoryDelta(
    "printing",
    historyDelta("printing", previous, next),
  );
...
test("2000 maximum-length multilingual work titles fit the shared byte cap", () => {
  const work = Array.from({ length: 2000 }, (_, i) => ({
    id: String(i),
    title: "界".repeat(500),
    date: null,
  }));
  assert.doesNotThrow(() =>
    serializeWrite(validateAppDataResource("work-tasks", work)),
  );
  assert.throws(
    () => serializeWrite({ text: "界".repeat(3_000_000) }),
    /8 MiB/,
  );
});
test("uncheck followed by deleting an active completion explicitly removes server-retained history", () => {
    ...
    lists: [
      {
        id: "list",
        title: "List",
        tasks: [{ id: "task", title: "Done", completedAt }],
      },
    ],
  };
  ...
  assert.throws(
    () =>
      validateHistoryDelta("todos", {
        ...delta,
        history: { upsert: [], remove: Array(2001).fill("task") },
      }),
    /2000/,
  );
});
