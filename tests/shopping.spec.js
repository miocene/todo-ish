...
test.beforeEach(async ({ page, appData }) => {
  appData.set("shopping", {
    tasks: [{ id: "manual", title: "Milk", completed: false }],
    history: [],
  });
...
test("purchase deletion preserves stock and activity; inventory failures recover without double counting", async ({
  ...
  await page.reload();
  await expect
    .poll(() => appData.get("filament-inventory")[filamentId])
    .toBe(3);
  await expect(materialRow(page, "filament")).toHaveCount(0);
  await page.goto("/profile");
  await expect(
    page.getByText(appData.get("shopping").history[0].title, { exact: true }),
  ).toHaveCount(1);
  expect(appData.validationErrors).toEqual([]);
});

test("manual editing restores titles and Enter skips generated rows", async ({
  page,
  appData,
}) => {
  ...
  await title.fill(" ");
  await page
    .getByRole("heading", { name: "Shopping cart", exact: true })
    .click();
  ...
  await expect(
    page.getByRole("checkbox", { name: "Complete untitled task" }),
  ).toBeDisabled();
...
test("manual completion deletion retains activity and uncheck deletion removes it", async ({
  page,
  appData,
}) => {
  await page
    .getByRole("checkbox", { name: "Complete Milk", exact: true })
    .check();
  await page
    .getByRole("button", {
      name: "Remove Milk from shopping list",
      exact: true,
    })
    .click();
  await expect
    .poll(() => appData.get("shopping").history?.map((item) => item.title))
    .toEqual(["Milk"]);
  await page.reload();
  ...
  await page
    .getByRole("button", {
      name: "Remove Bread from shopping list",
      exact: true,
    })
    .click();
  await page.reload();
...
test("new shortages do not overwrite the quantity of earlier purchases", async ({
  page,
  appData,
}) => {
  ...
  await expect
    ...
    .toBe(0);
  await expect(
    page.getByRole("status").filter({ hasText: "Inventory is now zero" }),
  ).toBeVisible();
});

test("Work and project deletion keep completed activity after reload", async ({
  page,
  appData,
}) => {
  ...
  await page
    .getByRole("button", { name: /Remove Finished work|Delete Finished work/ })
    .click();
  await expect.poll(() => appData.get("work-tasks")[0]?.archived).toBe(true);
  await page.reload();
  await expect(
    page.getByRole("checkbox", { name: "Complete Finished work" }),
  ).toHaveCount(0);
  appData.set("printing", {
    projects: [
      {
...
test("catalog refresh preserves a manual draft and a pending completion move", async ({
  page,
}) => {
  const pending = [];
  await page.route("**/api/catalogs/**", (route) => {
  ...
  await expect(
    page.locator(".shopping-card .task-item").last().getByRole("checkbox"),
  ).toBeChecked();
  await expect(page.locator(".shopping-card textarea").first()).toHaveValue(
    "During loading",
  );
});

test("rapid purchase toggles and a failed shopping save recover without duplicate stock", async ({
  page,
  appData,
}) => {
  ...
  await expect
    .poll(
      () => appData.get("shopping").tasks.filter((item) => item.source).length,
    )
    .toBe(1);
  expect(appData.get("filament-inventory")[filamentId]).toBe(3);
});

test("removing the last item focuses Add and blank drafts leave an empty card", async ({
  page,
  appData,
}) => {
  ...
  await advisory((expect) =>
    expect(page.locator(".shopping-card textarea")).toBeFocused(),
  );
  await add.focus();
