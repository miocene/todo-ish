...
const selectedCard = (page) =>
  page.getByRole("region").filter({ has: page.locator("time[datetime]") });
const backlogCard = (page) =>
  page.getByRole("region", { name: "Backlog", exact: true });
const title = (card) =>
  card.getByRole("textbox", { name: "Task title", exact: true });
const task = (id, date = null, checkedAt) => ({
  id,
  title: id,
  date,
  ...(checkedAt ? { checkedAt } : {}),
...
test("Work: rolled-over tasks can be corrected to Friday and persist as completed", async ({
  page,
}) => {
  ...
  await selectedCard(page)
    .getByRole("button", { name: "Move Friday work to backlog" })
    .click();
  ...
  await expect(
    page.getByRole("checkbox", { name: "Complete Friday work" }),
  ).toBeChecked();
  await expect
    .poll(() => data.get("work-tasks")[0])
    .toMatchObject({ date: FRIDAY, checkedAt: expect.any(String) });
  await page.reload();
  await expect(
    page.getByRole("checkbox", { name: "Complete Friday work" }),
  ).toBeChecked();
  ...
  await expect(
    page.locator(`.activity-day:has(header time[datetime="${FRIDAY}"])`),
  ).toContainText("Friday work");
  await expect(
    page.locator(`.activity-day:has(header time[datetime="${TODAY}"])`),
  ).toHaveCount(0);
  await expect(
...
test("Work: history is viewable but assignments and title edits stop after three days", async ({
  page,
}) => {
  ...
  await expect(
    selectedCard(page).getByRole("button", { name: "Add task", exact: true }),
  ).toHaveCount(0);
  await expect(
    backlogCard(page).getByRole("button", { name: /^Move Backlog/ }),
  ).toHaveCount(0);
  await selectedCard(page)
    .getByRole("button", { name: "Delete Old work" })
    .click();
  await expect
    .poll(
      () =>
        data.get("work-tasks").find((item) => item.id === "Old work")?.archived,
    )
    .toBe(true);
  await advisory((expect) =>
    expect(
...
for (const date of [FRIDAY, "2026-09-15"]) {
  test(`Work: checking a backlog task while viewing ${date} completes it today`, async ({
    page,
  }) => {
    const data = await openWork(page, [task("Finish now")], date);
    await backlogCard(page)
      .getByRole("checkbox", { name: "Complete Finish now" })
      .click();
    await expect
      .poll(() => data.get("work-tasks")[0])
      .toMatchObject({ date: TODAY, checkedAt: expect.any(String) });
    await expect(title(selectedCard(page))).toHaveCount(0);
    await advisory((expect) =>
      expect(
        backlogCard(page).getByRole("button", { name: "Add backlog task" }),
      ).toBeFocused(),
    );
    await page.getByRole("button", { name: "Today", exact: true }).click();
    await expect(
      page.getByRole("checkbox", { name: "Complete Finish now" }),
    ).toBeChecked();
  });
}

for (const date of [TODAY, "2026-09-15"]) {
  test(`Work: pinning to ${date} schedules an unchecked task and unpinning returns it`, async ({
    page,
  }) => {
    ...
    await page.reload();
    await selectedCard(page)
      .getByRole("button", { name: "Move Plan to backlog" })
      .click();
    await advisory((expect) => expect(title(backlogCard(page))).toBeFocused());
...
test("Work: past-day creation starts completed and title edits persist", async ({
  ...
  await expect
    .poll(() => data.get("work-tasks")[0].title)
    .toBe("Corrected title");
});

test("Work: direct URLs obey history and future bounds and malformed dates return today", async ({
  page,
}) => {
  await openWork(
    page,
    [task("Older history", "2026-08-01", "2026-08-01T12:00:00Z")],
    "2026-07-01",
  );
  ...
  await expect(
    page.getByRole("button", { name: /Next \d days/ }),
  ).toBeDisabled();
...
test("Work: future completions do not hide today or the three-day backdating window", async ({
  page,
}) => {
  await openWork(page, [
    task("Future work", "2026-09-15", "2026-09-14T12:00:00Z"),
  ]);
  await selectDay(page, FRIDAY);
  await expect(
    selectedCard(page).getByRole("button", { name: "Add task", exact: true }),
...
test("Work: row navigation preserves selection and Today restores the visible range", async ({
  page,
}) => {
  await openWork(page);
...
test("Work: day type changes persist without changing task assignment", async ({
  page,
}) => {
  const data = await openWork(page, [task("Plan", TODAY)]);
  await page
    .getByRole("combobox", { name: /^Change day type/ })
    .selectOption("pto");
...
test("Work: completion reorders after a delay and unchecking cancels the move", async ({
  page,
}) => {
...
test("Work: colors are created only for displayed cards and persist", async ({
  page,
}) => {
  const data = await openWork(page, [
    task("History", "2026-08-01", "2026-08-01T12:00:00Z"),
  ]);
  await expect
    .poll(() => Object.keys(data.get("colors") ?? {}).sort())
    .toEqual([`work-day:${TODAY}`]);
  const color = await selectedCard(page).evaluate((element) =>
    element.style.getPropertyValue("--color"),
  );
  await selectDay(page, FRIDAY);
  await expect
    .poll(() =>
      CARD_COLORS.includes(data.get("colors")?.[`work-day:${FRIDAY}`]),
    )
    .toBe(true);
  await page.getByRole("button", { name: "Today", exact: true }).click();
  expect(
    await selectedCard(page).evaluate((element) =>
      element.style.getPropertyValue("--color"),
    ),
  ).toBe(color);
  await page.reload();
...
test("Work: root and browser history preserve date selection and Today removes the query", async ({
  page,
}) => {
  await openWork(page);
  await page.goto("/");
...
test.describe("Work touch and local dates", () => {
  ...
  test("Work: local midnight preserves backdated completions and rolls only unfinished work", async ({
    ...
    const data = appDataByPage.get(page);
    data.set("work-tasks", [
      task("Friday done", FRIDAY, `${TODAY}T12:00:00Z`),
      task("Carry forward", TODAY),
    ]);
    await page.clock.install({ time: new Date("2026-09-14T21:59:50Z") });
    await page.goto(`/work?date=${FRIDAY}`);
    await expect(
      selectedCard(page).getByRole("checkbox", {
        name: "Complete Friday done",
      }),
    ).toBeChecked();
    ...
    await expect
      .poll(
        () =>
          data.get("work-tasks").find((item) => item.id === "Carry forward")
            .date,
      )
      .toBe("2026-09-15");
    expect(
      data.get("work-tasks").find((item) => item.id === "Friday done").date,
    ).toBe(FRIDAY);
    await page.reload();
    await expect(
      selectedCard(page).getByRole("checkbox", {
...
test("Work: stale duplicate drafts clear on startup without changing saved tasks", async ({
  page,
}) => {
  ...
  page.on("request", (request) => {
    if (
      request.method() === "PUT" &&
      request.url().endsWith("/api/data/work-tasks")
    )
      writes.push(request);
  });
      ...
      page.evaluate(
        () =>
          Object.keys(globalThis.localStorage).filter((key) =>
            key.startsWith("done-ish.pending-write.v1:"),
          ).length,
      ),
    )
    .toBe(0);
