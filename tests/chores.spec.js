...
test("monthly picker fits mobile and clamps dates to month-end", async ({
  ...
  await expect(
    dialog.getByRole("checkbox", { name: "Day 31", exact: true }),
  ).toBeDisabled();
  ...
  await page.reload();
  await page
    .getByRole("button", { name: "Edit Replace filter", exact: true })
    .click();
  await expect(
    dialog.getByRole("checkbox", { name: "Day 31", exact: true }),
  ).toBeChecked();
...
test("daily completion repeats after midnight and deletion removes both cards", async ({
  page,
  appData,
}) => {
...
test("legacy chores render, future chores stay hidden, and title-only edits preserve overdue dates", async ({
  ...
  await expect(page.locator(".chores-due").getByRole("checkbox")).toHaveCount(
    2,
  );
  await page
    .getByRole("button", { name: "Edit past chore", exact: true })
    .click();
  const dialog = page.getByRole("dialog");
  await expect(dialog.getByRole("combobox", { name: "Frequency" })).toHaveValue(
    "day",
  );
  await expect(dialog.locator(".chore-preview")).toContainText(
  ...
  await page.clock.fastForward(24 * 60 * 60 * 1000);
  await expect(
    page.getByRole("checkbox", { name: "Complete future chore", exact: true }),
  ).toBeVisible();
  expect(errors).toEqual([]);
});

test("overdue recurrence edits retain the due date and no-op submits do not write", async ({
  page,
  appData,
}) => {
  appData.set("chores", {
  ...
  page.on("request", (request) => {
    if (request.method() === "PUT" && request.url().endsWith("/chores"))
      writes++;
  });
  const dialog = page.getByRole("dialog");
  ...
  await dialog.getByRole("button", { name: "Save", exact: true }).click();
  await expect
    .poll(() => appData.get("chores").tasks[0].schedule?.interval)
    .toBe(2);
  expect(appData.get("chores").tasks[0].nextDue).toBe("2026-01-30");
  await page
    .getByRole("checkbox", { name: "Complete Overdue", exact: true })
    .check();
  await page.clock.fastForward(24 * 60 * 60 * 1000);
  await expect
    .poll(() => appData.get("chores").tasks[0].nextDue)
...
test("uncertain legacy rules require confirmation and title limits match the API", async ({
  page,
  appData,
}) => {
  appData.set("chores", {
  ...
  await page.reload();
  await expect(
    page.locator(".chores-all").getByText("Whenever needed", { exact: true }),
  ).toBeVisible();
  await page
    .getByRole("button", { name: "Edit Old chore", exact: true })
    .click();
  const dialog = page.getByRole("dialog");
  await expect(
    dialog.getByText(/Previous rule: Whenever needed/),
  ).toBeVisible();
...
test("failed chore saves recover after reload and deletion cancels a pending reorder", async ({
  page,
  appData,
}) => {
  ...
  await expect
    ...
    .toBe("Recover chore");
  await page
    .getByRole("checkbox", { name: "Complete Recover chore", exact: true })
    .check();
  await page
    .getByRole("button", { name: "Delete Recover chore", exact: true })
    .click();
  await page.clock.runFor(600);
  await expect.poll(() => appData.get("chores").tasks).toEqual([]);
...
test("multiple completions survive reload without resending unchanged history", async ({
  page,
  appData,
}) => {
  ...
  await dialog.getByRole("button", { name: "Add chore", exact: true }).click();
  await page
    .getByRole("checkbox", { name: "Complete Sweep twice", exact: true })
    .check();
  await expect.poll(() => appData.get("chores").tasks[0]?.completed).toBe(true);
  await page.clock.fastForward(24 * 60 * 60 * 1000);
  await expect(
    page.getByRole("checkbox", { name: "Complete Sweep twice", exact: true }),
  ).not.toBeChecked();
  await page
    .getByRole("checkbox", { name: "Complete Sweep twice", exact: true })
    .check();
  await expect.poll(() => appData.get("chores").tasks[0]?.completed).toBe(true);
  await page
    .getByRole("button", { name: "Delete Sweep twice", exact: true })
    .click();
  await expect.poll(() => appData.get("chores").history?.length).toBe(2);
  await page.reload();
  const requestPromise = page.waitForRequest(
    (request) =>
      request.method() === "PUT" && request.url().endsWith("/chores"),
  );
