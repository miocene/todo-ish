    ...
    lists: [
      { id: "general", title: "General", color: "#2765EC", tasks: [] },
      {
        ...
        tasks: [
          ...
          { id: "unfinished", title: "Unfinished task", completed: false },
        ],
      },
    ],
  });
...
test("cards use the shared kebab menu and General has one direct action", async ({
  page,
}) => {
  const cards = page.locator(".todo-list-card");
  await expect(cards).toHaveCount(2);
  await expect(
    page.getByRole("navigation", { name: "Todo lists" }),
  ).toHaveCount(0);
  await expect(page.locator(".page-header").getByRole("button")).toHaveText([
    "New list",
  ]);
  ...
  await expect(
    home.getByLabel("Actions for Home", { exact: true }),
  ).toBeVisible();
  await expect(
    home.getByRole("button", { name: "Delete Home list", exact: true }),
  ).toBeHidden();
  await home
    .getByRole("button", { name: "Collapse Home", exact: true })
    .click();
  await expect(home.getByRole("textbox").first()).toBeHidden();
  await listAction(page, "Home", "Add task to Home");
  await advisory((expect) =>
    expect(home.getByRole("textbox").nth(1)).toBeFocused(),
  );
...
test("deleting a list retains only completed activity without its list name", async ({
  ...
  expect(appData.get("todos").history.map((task) => task.title)).toEqual([
    "Finished task",
  ]);
  await page.reload();
  await page.goto("/profile");
  const day = page.locator(
    '.activity-day:has(header time[datetime="2026-02-02"])',
  );
...
test("completion followed by list deletion survives a failed save and reload", async ({
  page,
  appData,
}) => {
  appData.setWriteFailure("todos", 503);
  await page
    .getByRole("checkbox", { name: "Complete Unfinished task", exact: true })
    .check();
  await listAction(page, "Home", "Delete Home list");
...
test("individual task deletion persists, restores focus, and preserves only completed activity", async ({
  ...
  await page
    .getByRole("button", { name: "Delete Unfinished task", exact: true })
    .click();
  await advisory((expect) =>
    expect(page.locator("#todo-list-home").getByRole("textbox")).toBeFocused(),
  );
  await page
    .getByRole("button", { name: "Delete Finished task", exact: true })
    .click();
  await advisory((expect) =>
    expect(page.getByLabel("Actions for Home", { exact: true })).toBeFocused(),
  );
  await expect
    .poll(
      () => appData.get("todos").lists.find((list) => list.id === "home").tasks,
    )
    .toEqual([]);
  expect(appData.get("todos").history.map((task) => task.title)).toEqual([
    "Finished task",
...
test("items in General can be removed, including during pending completion reordering", async ({
  page,
  appData,
}) => {
  const general = page.locator("#todo-list-general");
  ...
  await general
    .getByRole("checkbox", { name: "Complete General task", exact: true })
    .check();
  await general
    .getByRole("button", { name: "Delete General task", exact: true })
    .click();
  await page.clock.runFor(600);
  await expect
    .poll(
      () =>
        appData.get("todos").lists.find((list) => list.id === "general").tasks,
    )
    .toEqual([]);
  await expect
    .poll(() =>
      appData
        .get("todos")
        .history?.some((task) => task.title === "General task"),
    )
    .toBe(true);
  await advisory((expect) =>
    expect(
      general.getByRole("button", { name: "Add task to General", exact: true }),
    ).toBeFocused(),
  );
});

test("titles restore on blank blur and valid edits survive failed saves", async ({
  page,
  appData,
}) => {
...
test("new tasks and Enter stay before completed tasks; blank drafts cannot complete", async ({
  page,
  appData,
}) => {
  const home = page.locator("#todo-list-home");
  ...
  await expect(
    home.getByRole("checkbox", { name: "Complete untitled task" }),
  ).toBeDisabled();
    ...
    test(`uncheck then delete ${removeList ? "list" : "item"} removes activity ${offline ? "offline" : "before save"}`, async ({
      ...
      await expect(
        page.getByRole("checkbox", {
          name: "Complete Finished task",
          exact: true,
        }),
      ).toBeChecked();
      await page.clock.pauseAt(new Date("2026-02-02T14:01:00"));
      await page
        .getByRole("checkbox", { name: "Complete Finished task", exact: true })
        .uncheck();
...
test("editing then deleting a completed task retains its newest title once", async ({
  page,
  appData,
}) => {
  await page.clock.pauseAt(new Date("2026-02-02T14:01:00"));
  ...
  await page
    .getByRole("button", { name: "Delete Updated completion", exact: true })
    .click();
  await page.clock.runFor(600);
  await expect
    .poll(() => appData.get("todos").history?.map((task) => task.title))
    .toEqual(["Updated completion"]);
  await page.reload();
  await page.goto("/profile");
  await expect(
    page.getByText("Updated completion", { exact: true }),
  ).toHaveCount(1);
});

test("new list modal supports cancellation and focuses the new Add action", async ({
  page,
}) => {
  ...
  await name.fill("   ");
  await expect(
    dialog.getByRole("button", { name: "Create list" }),
  ).toBeDisabled();
  ...
  await advisory((expect) =>
    expect(
      page.getByLabel("Actions for Travel", { exact: true }),
    ).toBeFocused(),
  );
});

test("list and task limits prevent unsavable additions", async ({
  page,
  appData,
}) => {
  appData.set("todos", {
