...
`tests/catalog-capacity.spec.js` verifies full catalogs and a ten-task project editor; `tests/activity.spec.js` covers a year with 2,000 history entries in Chromium. The Activity smoke test uses 14 entries across seven days on all three browsers, with touch taps on mobile Safari. Capacity checks run once; browser smoke tests cover navigation and interaction.
