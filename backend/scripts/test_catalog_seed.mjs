...
test("catalog seed data maps the browser snapshots to database columns", () => {
  ...
  const filamentSnapshot = JSON.parse(
    snapshot("bambu-filaments.snapshot.json"),
  );
  ...
  assert.equal(
    data.filament.sha256,
    digest(snapshot("bambu-filaments.snapshot.json")),
  );
  ...
  const listing = data.filament.rows.find(
    (entry) => entry.color_name === "Catalog listing",
  );
...
test("catalog seed SQL is transactional, idempotent, role-scoped, and self-validating", () => {
  ...
  assert.equal(
    sql,
    buildCatalogSeed(),
    "The generated SQL must be deterministic",
  );
});
