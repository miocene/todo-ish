...
const snapshot = (name) =>
  JSON.parse(readFileSync(new URL(`catalogs/${name}`, root), "utf8"));
...
test("catalogue snapshots keep only the application data contract", () => {
  ...
  assert.deepEqual(keys(bambuSnapshot.entries), [
    "color",
    "family",
    "id",
    "productCode",
  ...
  assert.deepEqual(keys(dmcSnapshot.entries), [
    "color",
    "colorName",
    "link",
    "number",
...
test("Bambu catalogue is complete, unique, and renderable", () => {
  assert.ok(
    bambuFilamentCatalog.length >= 250,
    "Expected the complete Bambu catalogue snapshot",
  );
  assert.deepEqual(duplicates(bambuFilamentCatalog.map((item) => item.id)), []);

  for (const item of bambuFilamentCatalog) {
    assert.ok(
      item.id && item.family && item.color && item.swatch,
      `Incomplete Bambu row: ${item.id || "unknown"}`,
    );
    assert.match(
      item.swatch,
      /^(?:https?:\/\/|#[0-9a-f]{3,8}$)/i,
      `Invalid Bambu swatch: ${item.id}`,
    );
    if (!item.productCode)
      assert.equal(
        item.color,
        "Catalog listing",
        `Missing product code: ${item.id}`,
      );
  }
...
test("DMC catalogue keeps canonical Threadcolors data and Breibrink links", () => {
  assert.ok(
    dmcFlossCatalog.length >= 440,
    "Expected the complete Threadcolors DMC table",
  );
  assert.deepEqual(duplicates(dmcFlossCatalog.map((item) => item.number)), []);
  for (let number = 1; number <= 35; number++) {
    const thread = dmcFlossCatalog.find(
      (item) => item.number === String(number).padStart(2, "0"),
    );
  ...
  for (const item of dmcFlossCatalog) {
    ...
    assert.match(
      item.color,
      /^#[0-9A-F]{6}$/,
      `Invalid DMC colour: ${item.number}`,
    );
  }

  assert.ok(
    dmcFlossCatalog.filter((item) => item.link).length >= 440,
    "Breibrink links should cover the DMC table",
  );
