...
remote_directory=$(setting PI_APP_DIR todo-app)
public_api_bind_address=$(setting PI_LAN_ADDRESS '')
adopt_legacy_checksums=$(setting PI_ADOPT_LEGACY_CHECKSUMS 0)
...
case "$adopt_legacy_checksums" in 0|1) ;; *) echo 'PI_ADOPT_LEGACY_CHECKSUMS must be 0 or 1' >&2; exit 2;; esac
echo "Legacy migration checksum adoption: $adopt_legacy_checksums"
...
ssh "$ssh_target" "MIGRATION_ADOPT_LEGACY_CHECKSUMS=$adopt_legacy_checksums sh '$release_directory/backend/scripts/activate_release.sh' '$remote_directory' '$revision' '$public_api_bind_address'"
