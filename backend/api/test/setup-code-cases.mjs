...
export async function verifySetupCodes(repository, userId) {
  ...
  const issue = (token, expiresAt) =>
    repository.provisionUser({ user, setupCode: { tokenHash: hash(token), expiresAt } });
  ...
  assert.equal((await repository.userForSetupCode(hash("active-code"))).id, userId);
  const generated = [];
  let sequence = 0;
    ...
    },
    {
      ...
      async verifyRegistrationResponse({ response }) {
        return {
          verified: true,
          registrationInfo: {
            credential: { id: response.id, publicKey: Uint8Array.from([1, 2, 3]), counter: 0 },
  ...
  const one = await service.registrationOptions({ bootstrapToken: "active-code" });
  const two = await service.registrationOptions({ bootstrapToken: "active-code" });
  ...
  assert.equal((await service.session(result.cookies[1].split(";")[0])).user.id, userId);
  ...
  await assert.rejects(service.registrationOptions({ cookieHeader: "" }), (error) => error.statusCode === 401);
  await service.registrationOptions({ cookieHeader: result.cookies[1].split(";")[0] });
