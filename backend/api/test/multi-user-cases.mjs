...
export async function verifyMultiUserData(repository, first, second) {
  const write = async (user, resource, value) => {
    ...
    return repository.replace(resource, validateAppDataResource(resource, value), state.revisions[resource], user);
  };
  ...
  for (const user of [first, second]) {
    ...
    assert.equal(state.pages.todos.lists.find((list) => list.id === "general").tasks[0].title, user);
  ...
  assert.equal((await repository.read(first)).pages.printing.projects.length, 1);
  ...
  await write(first, "chores", {
    ...
    tasks: [{ id: "shared-chore", title: "Plants", details: "Weekly", nextDue: "2026-09-09" }],
  });
  ...
  assert.equal((await repository.read(first)).pages.chores.tasks[0].completed, true);
  ...
  await write(first, "shopping", { tasks: [{ id: "shared-item", title: "Milk" }, purchase], history: [] });
  ...
  assert.equal(firstShopping.tasks.find((item) => item.id === "shared-item").completed, true);
  ...
  assert.equal((await repository.read(first)).pages.shopping.history, undefined);
  await write(first, "shopping", { tasks: [], history: [purchase] });
  assert.deepEqual(
    (await repository.read(second)).pages.shopping.history.map((item) => item.id),
    [secondPurchase.id],
  );

  const stale = await repository.read(first);
  await write(second, "shopping", { tasks: [{ id: "new-shared", title: "Bread" }], history: [secondPurchase] });
  await assert.rejects(
    repository.replace("shopping", { tasks: [] }, stale.revisions.shopping, first),
    AppDataRevisionConflictError,
  );
  assert.equal((await repository.read(first)).pages.shopping.tasks[0].title, "Bread");
  ...
  await assert.rejects(repository.replace("shopping", { tasks: [] }, 0), /authenticated user ID/);
}
