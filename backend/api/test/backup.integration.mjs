...
const mode = process.env.TEST_BACKUP_CONTAINER
  ? "docker"
...
test(
  "daily dump replaces only after success and restores the latest data",
  {
    skip:
      !mode &&
      "Set TEST_PG_BIN or TEST_BACKUP_CONTAINER to test database backups",
  },
  async (t) => {
    ...
    const backupScript = fileURLToPath(
      new URL("../../scripts/database_backup.sh", import.meta.url),
    );
    const run = (overrides = {}) =>
      execute("sh", [backupScript, directory], {
    ...
    await assert.rejects(
      run({ DB_NAME: `missing_${suffix}` }),
      /does not exist/,
    );
    assert.deepEqual(
      await readFile(dump),
      previous,
      "a failed dump preserves the last good backup",
    );
    await installer.query(
      "UPDATE work_tasks SET title='Latest 復元 🙂' WHERE id='backup-task'",
    );
    assert.equal((await run()).stdout.trim(), dump);
    assert.equal(
      await readFile(oldBackup, "utf8"),
      "previous backup",
      "older backups are never removed",
    );
    ...
    await admin.query(
      `CREATE DATABASE "${restoredDatabase}" TEMPLATE template0`,
    );
    const args = [
      "pg_restore",
      "-U",
    ...
    const command =
      mode === "docker"
        ? ["docker", "exec", "-i", environment.DB_CONTAINER, ...args]
        : args;
    ...
    assert.equal(
      (
        await restored.query(
          "SELECT title FROM work_tasks WHERE id='backup-task'",
        )
      ).rows[0].title,
      "Latest 復元 🙂",
    );
    assert.equal(
      (
        await restored.query(
          "SELECT count(*)::integer AS count FROM auth_users",
        )
      ).rows[0].count,
      1,
    );
