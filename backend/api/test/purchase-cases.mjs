...
export async function verifyAtomicPurchases(repository, first, second) {
  const write = async (user, resource, value) =>
    repository.replace(
      resource,
      value,
      (await repository.read(user)).revisions[resource],
      user,
    );
  ...
  for (const user of [first, second])
    await write(user, "shopping", { tasks: [], history: [] });
  ...
  await write([first, second][loser], "shopping", {
    tasks: [buys[loser]],
    history: [],
  });
  assert.equal(
    (await repository.read(first)).inventories.filament.concurrent,
    2,
  );
  ...
  assert.equal(
    (await repository.read(first)).inventories.filament.concurrent ?? 0,
    0,
  );
  await assert.rejects(
    write(first, "shopping", { tasks: [buys[0]], history: [] }),
    /cannot be changed or reused/,
  );
  assert.equal(
    (await repository.read(second)).pages.shopping.tasks[0].id,
    buys[1].id,
  );
  ...
  await assert.rejects(
    write(first, "shopping", { tasks: failed, history: [] }),
    /inventory limit/,
  );
