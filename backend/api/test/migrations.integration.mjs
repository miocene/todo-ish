...
test("actual migration runner: fresh, rerun, upgrade, concurrent, rollback and checksum protection", async (t) => {
  ...
  await cp(new URL("../../database/migrations/", import.meta.url), directory, {
    recursive: true,
  });
  ...
  await client.query(
    "INSERT INTO auth_users(id, username, display_name) VALUES ('palette', 'palette', 'Palette')",
  );
  await client.query(
    "INSERT INTO colors(user_id,id,color) VALUES ('palette','backlog','#2765EC')",
  );
  ...
  for (const table of [
    ...
    const result = await client.query(
      `SELECT color FROM ${table} WHERE color IS NOT NULL`,
    );
  ...
  const catalog = await createCatalogRepository(client).floss({
    query: "",
    limit: 500,
    offset: 0,
  });
  ...
  await writeFile(
    extra,
    "CREATE TABLE runner_upgrade(id integer); SELECT pg_sleep(0.1);",
  );
  await Promise.all([
    runMigrations(client, directory, options),
    runMigrations(await connect(), directory, options),
  ]);
  assert.equal(
    (
      await client.query(
        "SELECT count(*)::integer AS count FROM schema_migrations WHERE filename='9998_upgrade.sql'",
      )
    ).rows[0].count,
    1,
  );
  ...
  assert.equal(
    (await client.query("SELECT to_regclass('rolled_back') AS value")).rows[0]
      .value,
    null,
  );
  ...
  assert.equal(
    (
      await client.query(
        "SELECT count(*)::integer AS count FROM schema_migrations WHERE checksum IS NULL",
      )
    ).rows[0].count,
    0,
  );
