...
export async function runMigrations(
  ...
  if (!/^[a-z_][a-z0-9_]*$/.test(runtimeRole))
    throw new Error("Unsafe migration runtime role");
  const names = (await readdir(directory))
    .filter((name) => name.endsWith(".sql"))
    .sort();
  if (
    !names.length ||
    names.some((name) => !/^\d{4}_[A-Za-z0-9_.-]+\.sql$/.test(name))
  )
  ...
  try {
    ...
    if (!applied.length) {
      ...
      if (result.rows[0].count) {
        if (result.rows[0].count !== 4)
          throw new Error("The catalog schema is incomplete");
        if (!adoptLegacy)
          throw new Error(
            "Review the existing catalog schema, then explicitly adopt legacy checksums",
          );
    ...
    for (const row of applied) {
      ...
      if (!source)
        throw new Error(`Applied migration is missing: ${row.filename}`);
      ...
      if (!row.checksum && !adoptLegacy)
        throw new Error(
          `Review existing migrations and set MIGRATION_ADOPT_LEGACY_CHECKSUMS=1 once: ${row.filename}`,
        );
    }
    ...
    for (const migration of migrations) {
      ...
      try {
        if (row)
          log(`Adopting reviewed legacy checksum: ${migration.filename}`);
        else {
          log(`Applying: ${migration.filename}`);
          await client.query(
            migration.sql.replaceAll('"todo_runtime"', `"${runtimeRole}"`),
          );
        }
        await client.query(
