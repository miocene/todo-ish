  ...
  return transaction(
    ...
    async (client) => {
      ...
      const rows = (resource, sql, values = [], historyQuery = false) =>
        ...
        (!onlyHistory || historyQuery) &&
        (history !== "omit" || !historyQuery)
          ? queryRows(client, sql, values)
          : [];
      const mixedRows = (resource, sql) =>
        resources.includes(resource) ? queryRows(client, sql) : [];
      const ownerRows = await queryRows(
        client,
        "SELECT id FROM auth_users ORDER BY created_at, id LIMIT 1",
      );
      ...
      const colorRows = await rows(
        "colors",
        "SELECT id, color FROM colors ORDER BY id",
      );
      ...
      const currentChores = new Set(
        choreRows.map((row) => `${row.id}:${row.nextDue}`),
      );
      ...
      const shoppingTask = (item) => ({
        ...
        ...(item.source && {
          ...
          [item.source === "filament-shortage" ? "filamentId" : "flossId"]:
            item.catalogId,
        }),
      ...
      const filamentUsagesByItem = new Map(
        printingItemRows.map((item) => [item.id, []]),
      );
      for (const usage of filamentUsageRows) {
        filamentUsagesByItem.get(usage.itemId)?.push({
          ...
          label: usage.label,
          weightGrams:
            usage.weightGrams === null ? "" : Number(usage.weightGrams),
        });
      }
      const printingItemsByProject = new Map(
        printingProjectRows.map((project) => [project.id, []]),
      );
      ...
      const stitchThreadsByProject = new Map(
        stitchProjectRows.map((project) => [project.id, []]),
      );
      ...
      const state = {
        ...
        workStatuses: Object.fromEntries(
          workStatusRows.map((row) => [row.date, row.status]),
        ),
        ...
        pages: {
          chores: {
            ...
            occurrenceOrder: choreRows
              ...
              .sort(
                (first, second) =>
                  first.occurrencePosition - second.occurrencePosition,
              )
              .map((row) => row.id),
            tasks: choreRows
              .filter((row) => row.nextDue)
          ...
          todos: {
            ...(todoItemRows.some(
              (item) => !item.listId && item.completedAt,
            ) && {
          ...
          shopping: {
            ...
            ...(shoppingRows.some((item) => item.archived) && {
              history: shoppingRows
                .filter((item) => item.archived)
                .map(shoppingTask),
            }),
          },
          printing: {
            ...(historyFor("printing").length && {
              history: historyFor("printing"),
            }),
          ...
          crossStitch: {
            ...(historyFor("cross-stitch").length && {
              history: historyFor("cross-stitch"),
            }),
            projects: stitchProjectRows.map((project) => {
              const tasks = stitchThreadsByProject.get(project.id) ?? [];
              return {
                ...project,
                totalCrosses: tasks.reduce(
                  (total, item) => total + item.crosses,
                  0,
                ),
                tasks,
              };
        ...
        inventories: {
          filament: Object.fromEntries(
            filamentInventoryRows.map((row) => [row.catalogId, row.count]),
          ),
          floss: Object.fromEntries(
            flossInventoryRows.map((row) => [row.catalogId, row.count]),
          ),
        },
      };
      if (onlyHistory) {
        const resource = resources[0];
        const items = splitHistory(
          resource,
          resourceFromState(state, resource),
        ).history;
        return {
          userId,
          resource,
      ...
      for (const resource of APP_DATA_RESOURCES)
        if (!resources.includes(resource))
          setStateResource(state, resource, undefined);
      return state;
    },
  );
}
...
async function upsertRows(
  ...
  columns,
  rows,
  { keys = ["id"], updatedAt = true, preserve = [] } = {},
) {
...
async function replaceChores(client, data) {
  const occurrencePosition = new Map(
    data.occurrenceOrder.map((id, position) => [id, position]),
  );
  await upsertRows(
    client,
    "chores",
...
async function replaceProjectHistory(client, resource, data, patch) {
  ...
  if (!patch)
    await client.query({
      text: "DELETE FROM completed_project_tasks WHERE resource = $1",
      values: [resource],
    });
  await insertRows(
    client,
...
async function replacePrinting(client, data, _userId, patch = false) {
  ...
  await upsertRows(
    client,
    "printing_items",
    ["id", "project_id", "title", "completed_at", "position"],
    items,
  );
  await upsertRows(
    client,
...
const WRITERS = Object.freeze({
  ...
  colors: (client, colors) =>
    upsertRows(client, "colors", ["id", "color"], Object.entries(colors), {
      updatedAt: false,
    }),
  ...
  "filament-inventory": (client, data) =>
    replaceInventory(client, "filament_inventory", "spool_count", data),
  "floss-inventory": (client, data) =>
    replaceInventory(client, "floss_inventory", "skein_count", data),
});
...
async function removeHistory(client, resource, keys) {
  ...
  await client.query({
    text: `DELETE FROM ${table} WHERE ${predicate} AND ${key} = ANY($1::text[])`,
    values: [keys],
  });
}
...
async function replaceResource(
  ...
  return transaction(pool, "", userId, async (client) => {
    ...
    if (currentRevision !== expectedRevision) {
      throw new AppDataRevisionConflictError(
        resource,
        expectedRevision,
        currentRevision,
      );
    ...
    if (resource === "shopping")
      await adjustShoppingStock(client, data, userId, historyPatch);
    await WRITERS[resource](client, data, userId, Boolean(historyPatch));
    if (historyPatch?.remove.length)
      await removeHistory(client, resource, historyPatch.remove);
    if (historyPatch) {
      ...
      if (count[0].count > 100_000)
        throw new AppDataValidationError(
          "This resource has reached its 100,000-entry history limit.",
        );
    }
    ...
    revisions: (userId) =>
      transaction(pool, "READ ONLY", userId, async (client) => {
        const rows = await queryRows(
          client,
          "SELECT resource, revision FROM app_data_revisions ORDER BY resource",
        );
        return {
          userId,
          revisions: {
            ...
            ...Object.fromEntries(
              rows.map((row) => [row.resource, row.revision]),
            ),
          },
        };
      }),
