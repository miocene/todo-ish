...
const clientSource = sources
  ...
  .map(read)
  .join("\n");
assert.doesNotMatch(clientSource, /<style\s+scoped\b/i, "Component styles should remain in their CSS files");
...
const iconNames = [...iconSprite.matchAll(/<symbol id="([^"]+)"/g)].map(([, name]) => name);
assert.equal(new Set(iconNames).size, iconNames.length, "Icon IDs must be unique");
  ...
  plugins: [
    {
      ...
      generateBundle() {
        const modules = new Set([...this.getModuleIds()].map((id) => id.split("?")[0]));
        const unused = sources.filter((path) => !modules.has(fileURLToPath(new URL(path, root))));
        assert.deepEqual(unused, [], `Client files outside the application import graph:\n${unused.join("\n")}`);
      },
    },
  ],
});
console.log("Repository audit passed · imports resolve · client files are reachable · backend boundaries preserved");
