...
const items = snapshot.entries.map((item) => ({
  ...item,
  id: `dmc${item.number.toLocaleLowerCase()}`,
}));
floss.push(...items);
...
const baseline = (query) =>
  items
    .filter((item) =>
      [item.id, item.number, item.colorName]
        .join(" ")
        .toLocaleLowerCase()
        .includes(query),
    )
...
const optimized = (query) =>
  indexed.filter((item) => item.searchText.includes(query));
