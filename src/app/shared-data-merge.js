...
function merge(base, local, remote, field = "") {
  ...
  if (field === "completedAt" && !base && local && remote) return local < remote ? local : remote;
  if (Array.isArray(base) && Array.isArray(local) && Array.isArray(remote)) {
    ...
    if (field === "hiddenNavigation") {
      return [...new Set([...base, ...local, ...remote])].filter((id) =>
        local.includes(id) === base.includes(id) ? remote.includes(id) : local.includes(id),
      );
    }
    if (![...base, ...local, ...remote].every((item) => item && typeof item.id === "string")) return conflict;
    const key = (item) => (field === "history" && item.nextDue ? `${item.id}:${item.nextDue}` : item.id);
    const records = [base, local, remote].map((items) => new Map(items.map((item) => [key(item), item])));
    const result = [];
    for (const id of new Set([...records[1].keys(), ...records[2].keys(), ...records[0].keys()])) {
  ...
  if (
    ...
    local &&
    remote &&
    [base, local, remote].every((value) => typeof value === "object" && !Array.isArray(value))
  ) {
    const result = [];
...
export function mergeSharedData(resource, base, local, remote) {
  if (!SHARED_APP_DATA_RESOURCES.includes(resource) && resource !== "preferences") return undefined;
  const values = [base, local, remote].map((value) => {
    const normalized = validateAppDataResource(resource, value);
    if (resource === "chores" || resource === "shopping") normalized.history ??= [];
  ...
  if (result.tasks) {
    for (const item of [...result.tasks, ...result.history]) item.completed = Boolean(item.completedAt);
    if (result.occurrenceOrder) {
      const ids = new Set(result.tasks.map((item) => item.id));
      result.occurrenceOrder = [...new Set([...result.occurrenceOrder, ...ids])].filter((id) => ids.has(id));
    }
  }
