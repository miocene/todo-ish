...
const weekStart = (value) =>
  dateNumber(value) - ((calendarDate(value).getDay() + 6) % 7);
...
export function scheduleFromChore(task, today) {
  ...
  const intervalMatch =
    /^(?:every\s+)?(\d+)\s+(days?|weeks?|months?)(?:\s+on\s+(.+))?$/.exec(rule);
  const weeklyDays =
    /^(?:every(?:\s+other)?(?:\s+week)?(?:\s+on)?\s+)(.+)$/.exec(rule);
  const dayNames = (value) =>
    value
      ?.split(/,\s*|\s+and\s+/)
      .map((name) =>
        WEEKDAYS.findIndex((day) => day.toLowerCase() === name.trim()),
      );
  let frequency;
  ...
  else if (weeklyDays && dayNames(weeklyDays[1]).every((day) => day >= 0))
    frequency = "week";
  else return null;
  ...
  if (intervalMatch) {
    ...
    weeklyDays &&
    frequency === "week" &&
    !/^(weekly|every week)$/.test(rule)
  ) {
    schedule.weekdays = [...new Set(dayNames(weeklyDays[1]))].sort(
      (a, b) => a - b,
    );
...
export function choreScheduleLabel(schedule) {
  ...
  if (frequency === "week")
    return `${every} on ${weekdays.map((day) => WEEKDAYS[day].slice(0, 3)).join(", ")}`;
...
export function nextChoreDate(schedule, from) {
  ...
  let candidate = calendarDate(
    from < schedule.startDate ? schedule.startDate : from,
  );
  for (let step = 0; step < 366 * 100; step++) {
    ...
    if (schedule.frequency === "day") {
      matches =
        (dateNumber(iso) - dateNumber(schedule.startDate)) %
          schedule.interval ===
        0;
    } else if (schedule.frequency === "week") {
      matches =
        ((weekStart(iso) - weekStart(schedule.startDate)) / 7) %
          schedule.interval ===
          0 && schedule.weekdays.includes((candidate.getDay() + 6) % 7);
    } else {
      const monthOffset =
      ...
      matches =
        ...
        schedule.monthDays.some(
          (day) => Math.min(day, lastDay) === candidate.getDate(),
        );
    }
...
export function sameChoreSchedule(first, second) {
  ...
  if (
    ...
    return false;
  const field =
    first.frequency === "week"
      ? "weekdays"
      : first.frequency === "month"
        ? "monthDays"
        : null;
  return (
    !field ||
    (first[field].length === second[field].length &&
      first[field].every((day) => second[field].includes(day)))
  );
}
...
export function retainChoreCompletion(chores, task) {
  ...
  if (
    chores.history.some(
      (item) => item.id === task.id && item.nextDue === task.nextDue,
    )
  )
    return;
  chores.history.push({
...
export function completedChoreOccurrences(chores) {
  const entries = new Map(
    (chores.history ?? []).map((item) => [`${item.id}:${item.nextDue}`, item]),
  );
  for (const task of chores.tasks) {
    const key = `${task.id}:${task.nextDue}`;
    if (task.completedAt)
      entries.set(key, { ...task, details: choreDescription(task) });
    else entries.delete(key);
  }
  return [...entries.values()].map((item) => ({
    ...item,
    id: `${item.id}:${item.nextDue}`,
  }));
}
