    ...
    load() {
      ...
      request = (async () => {
        try {
          ...
          let total;
          do {
            const response = await fetchPage(`${path}?limit=500&offset=${loaded.length}`, {
            ...
            total = page.total;
            if (!page.items.length && loaded.length < total) throw new Error(`${label} pagination did not advance.`);
            loaded.push(...page.items);
          } while (loaded.length < total);
          if (loaded.length !== total || new Set(loaded.map((item) => item.id)).size !== loaded.length) {
