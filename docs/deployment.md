...
Set `PI_SSH_TARGET`, `PI_LAN_ADDRESS` and optionally `PI_APP_DIR` in `.env.local` or the command environment. Environment values take precedence. The deploy script also reads `PI_ADOPT_LEGACY_CHECKSUMS` from either location and prints its effective value before running checks. It parses these settings without executing `.env.local` as shell code. No production host is contacted by the quality or database integration tests.
