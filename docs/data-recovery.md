...
Automatic migration of the original account's old browser data remains supported. Original localStorage values stay in place after initialization and save acknowledgement. The app no longer creates additional legacy archives or offers saved-data downloads. Account JSON export, preview and restore tools have been removed; database backup and recovery are described in `docs/deployment.md`.
